    ...
    public JSONObject query(Map<String, Object> paramMap){
        try{
            ...
            if(limit != null || page != null){
                ...
                PageBean<Customer> pageBean = customerServiceFacade.listPage(pageParam, paramMap);
                List<Customer> customers = pageBean.getRecordList();
                JSONArray dataArray = new JSONArray();
                for(int i = 0; i < customers.size(); i++){
                    Map<String, Object> queryMap = new HashMap<>();
                    queryMap.put("IfDeleted", false);
                    queryMap.put("UserInfo", customers.get(i).getId());
                    queryMap.put("UserType", UserType.Customer);
                    List<User> users = userServiceFacade.listBy(queryMap);
                    if(users.size() > 1){
                        return getResultJSON("未知错误，"+getClassDesc()+"信息（id=" + customers.get(i).getId() + "）被多个登录信息关联");
                    }else if(users.size() == 0){
                        return getResultJSON("未知错误，"+getClassDesc()+"信息（id=" + customers.get(i).getId() + "）没有关联的登录信息");
                    }else{
                        JSONObject itemObject = (JSONObject) JSONObject.toJSON(customers.get(i));
                ...
                List<Customer> admins = customerServiceFacade.listBy(paramMap);
                JSONArray dataArray = new JSONArray();
