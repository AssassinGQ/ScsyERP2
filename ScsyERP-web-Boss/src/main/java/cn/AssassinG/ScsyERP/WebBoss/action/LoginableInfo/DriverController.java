...
public class DriverController extends LoginableBaseController<Driver> {
    ...
    @Override
    protected LoginableService<Driver> getLoginableService() {
        return this.driverServiceFacade;
    }
    ...
    @ResponseBody
    public JSONObject create(Driver driver, User user){
        return super.createImpl(driver, user);
    }
    ...
    public JSONObject query(Map<String, Object> paramMap){
        try{
            ...
            if(limit != null || page != null){
                ...
                PageBean<Driver> pageBean = driverServiceFacade.listPage(pageParam, paramMap);
                List<Driver> drivers = pageBean.getRecordList();
                JSONArray dataArray = new JSONArray();
                for(int i = 0; i < drivers.size(); i++){
                    Map<String, Object> queryMap = new HashMap<>();
                    queryMap.put("IfDeleted", false);
                    queryMap.put("UserInfo", drivers.get(i).getId());
                    queryMap.put("UserType", UserType.Driver);
                    List<User> users = userServiceFacade.listBy(queryMap);
                    if(users.size() > 1){
                        return getResultJSON("未知错误，"+getClassDesc()+"信息（id=" + drivers.get(i).getId() + "）被多个登录信息关联");
                    }else if(users.size() == 0){
                        return getResultJSON("未知错误，"+getClassDesc()+"信息（id=" + drivers.get(i).getId() + "）没有关联的登录信息");
                    }else{
                        JSONObject itemObject = (JSONObject) JSONObject.toJSON(drivers.get(i));
                ...
                List<Driver> admins = driverServiceFacade.listBy(paramMap);
                JSONArray dataArray = new JSONArray();
